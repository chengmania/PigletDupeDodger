...
// rebuilding fresh <a> elements on every broadcast created a window where a
// click on one could land on an anchor that gets replaced mid-click.
...
function rerender(): void {
  ...
  // The effective screen identity must mirror currentScreen()'s own routing
  // exactly: leaderboard bypasses the sign-in gate entirely, and everything
  // else falls back to the connect screen when signed out. Tracking this as
  // part of the mount key means a status change like connecting->connected
  // doesn't look like a screen change (so connect.ts's in-progress form
  // isn't wiped), while an actual screen change always is one.

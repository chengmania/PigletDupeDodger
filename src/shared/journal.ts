...
export type JournalEvent =
  ...
  | { type: 'op:join'; ts: string; call: string; name?: string; age18OrUnder?: boolean }
  ...
  | { type: 'bonus:set'; ts: string; bonusId: string; claim: BonusClaim };

export interface State {
  config: ClubConfig | null;
  operators: Map<string, Operator>;
  // key: 'GOTA' for the GOTA singleton slot, else `${band}|${mode}` (MAIN only).
...
export function reservationKey(station: StationKind, band: string, mode: Mode): string {
  return station === 'GOTA' ? 'GOTA' : `${band}|${mode}`;
}
  ...
  switch (event.type) {
    ...
    case 'op:join': {
      ...
      operators.set(event.call, {
        ...
        age18OrUnder: event.age18OrUnder ?? existing?.age18OrUnder,
    ...
    case 'op:leave': {
      ...
      // operator-list export (Rule 4.1.1.2), so this event is a no-op on
      // folded state today. Kept as a distinct event type for future use
      // (e.g. a host "kick" audit trail).
      return state;
    }
    case 'slot:reserve': {
    ...
    case 'bonus:set': {
      const bonuses = new Map(state.bonuses);
      bonuses.set(event.bonusId, event.claim);
